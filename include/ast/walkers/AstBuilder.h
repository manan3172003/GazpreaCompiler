...
public:
  ...
  std::any visitDotFloatLiteral(GazpreaParser::DotFloatLiteralContext *ctx) override;
  ...
  std::any visitFloatDotLiteral(GazpreaParser::FloatDotLiteralContext *ctx) override;
  std::any visitTupleLiteral(GazpreaParser::TupleLiteralContext *ctx) override;
  std::any visitMulDivRemDstarExpr(GazpreaParser::MulDivRemDstarExprContext *ctx) override;
...
private:
  ...
  createDefaultLiteral(const std::shared_ptr<types::DataTypeAst> &typeAst, antlr4::Token *token);
};
} // namespace gazprea::ast::walkers