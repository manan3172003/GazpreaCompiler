...
std::shared_ptr<GlobalScope> SymbolTable::getGlobalScope() {
  return globalScope;
}
void SymbolTable::setGlobalScope(std::shared_ptr<GlobalScope> scope) {
  globalScope = scope;
}
std::shared_ptr<Scope> SymbolTable::getCurrentScope() { return currentScope; }
void SymbolTable::setCurrentScope(std::shared_ptr<Scope> scope) {
  currentScope = scope;
}
